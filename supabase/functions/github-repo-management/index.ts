  ...
  try {
    ...
    if (action === 'fetch-repos') {
      ...
      if (connectionError || !connection) {
        return new Response(
          JSON.stringify({ error: 'GitHub connection not found', repos: [] }),
      ...
      try {
        ...
        if (!repos || !Array.isArray(repos)) {
          return new Response(
            JSON.stringify({ error: 'Invalid response from GitHub API', repos: [] }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 502 }
          );
        }
        
        // Log some info about the repos for debugging
        ...
        return new Response(
          JSON.stringify({ error: error.message || 'Failed to fetch repositories', repos: [] }),
    ...
    if (action === 'create') {
      ...
      try {
        ...
        if (!githubResponse.ok) {
          ...
          return new Response(
            JSON.stringify({ error: `GitHub API error: ${JSON.stringify(errorData)}` }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 502 }
          );
        }
        ...
        return new Response(
          JSON.stringify({ error: error.message || 'Failed to create GitHub repository' }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500 }
        );
      }
    }
    ...
    if (action === 'index-repo') {
      if (!projectId || !repoFullName) {
        return new Response(
          JSON.stringify({ error: 'Missing required fields: projectId and repoFullName are required' }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 }
        );
      }
      ...
      try {
        ...
        return new Response(
          JSON.stringify({ error: error.message || 'Failed to index repository' }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500 }
        );
      }
    }
