...
import { EnhancedChatMessage } from '@/types/chat-preferences';
...
export const chatMessagesService = {
  ...
  ): Promise<{ messages: EnhancedChatMessage[]; hasMore: boolean; nextCursor?: string }> {
    try {
      const { 
        cursor, 
   ...
   */
  async sendMessage(
    message: Omit<EnhancedChatMessage, 'id' | 'created_at' | 'updated_at'>,
    maxRetries: number = 3
  ): Promise<EnhancedChatMessage> {
    const attemptSend = async (retryCount: number): Promise<EnhancedChatMessage> => {
      try {
  ...
  async updateMessageStatus(
    messageId: string, 
    status: EnhancedChatMessage['status']
  ): Promise<EnhancedChatMessage> {
    try {
      const { data, error } = await supabase
        .from('chat_messages')
   ...
   */
  subscribeToMessages(sessionId: string, callback: (message: EnhancedChatMessage) => void) {
    return supabase
      ...
      .on(
        ...
        payload => {
          callback(payload.new as EnhancedChatMessage);
        }
      )
      .subscribe();
  }
