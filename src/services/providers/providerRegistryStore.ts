...
export const useProviderRegistry = create<ProviderRegistry>()(
  devtools(
    persist(
      (set, get) => ({
        ...initialState,
        ...
        verifyApiKey: async (id: string): Promise<boolean> => {
          const provider = get().providers.find(p => p.id === id);
          if (!provider) {
        ...
        getDefaultProvider: (category: ProviderCategoryType): Provider | null => {
          ...
          const defaultProvider = get().providers.find(p => 
            p.category === category && p.isDefault && p.isEnabled
          );
