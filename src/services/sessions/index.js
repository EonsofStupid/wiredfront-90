 ...
 */
export async function updateChatSession(sessionId, params) {
    try {
        const { data: userAuth } = await supabase.auth.getUser();
        if (!userAuth?.user) {
            return { success: false, error: new Error('User not authenticated') };
 ...
 */
export async function clearChatSessions(preserveSessionId = null) {
    try {
        const { data: userAuth } = await supabase.auth.getUser();
        if (!userAuth?.user) {
            return { success: false, error: new Error('User not authenticated') };
 ...
 */
export async function cleanupInactiveChatSessions() {
    try {
        const { data: userAuth } = await supabase.auth.getUser();
        if (!userAuth?.user) {
            return { success: false, error: new Error('User not authenticated') };
