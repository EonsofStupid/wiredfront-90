...
export class AnthropicProvider extends BaseProvider {
    ...
    async generateText(prompt, options = {}) {
        try {
            ...
            // Track the usage
            if (data?.usage) {
                this.trackUsage('text_generation', data.usage.total_tokens || 0, (data.usage.total_tokens || 0) * 0.00002 // Approximate cost calculation
                );
            }
    ...
    async prepareRAGContext(documents, query) {
        ...
        const contextChunks = documents.map((doc, index) => `<document id="${index + 1}">\n${doc.content}\n</document>`);
        return `
<system>
