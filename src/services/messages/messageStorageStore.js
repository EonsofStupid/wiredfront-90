...
export const useMessageStorage = create()(devtools(persist((set, get) => ({
    ...initialState,
    addMessage: (message) => {
        if (!message.id) {
    ...
    searchMessages: (query, sessionId) => {
        ...
        const filteredMessages = sessionId
            ? allMessages.filter(message => (message.chat_session_id === sessionId || message.sessionId === sessionId))
            : allMessages;
        return filteredMessages.filter(message => message.content.toLowerCase().includes(query.toLowerCase()));
    }
}), {
