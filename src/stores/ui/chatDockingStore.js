...
export const positionAtom = atomWithStorage('chat-position', defaultPosition);
export const dockedItemsAtom = atomWithStorage('chat-docked-items', {});
// Derived state atom
...
// Action atoms
export const setDockedAtom = atom(null, (get, set, docked) => {
    set(dockedAtom, docked);
});
export const toggleDockedAtom = atom(null, (get, set) => {
    set(dockedAtom, !get(dockedAtom));
});
export const setPositionAtom = atom(null, (get, set, position) => {
    set(positionAtom, position);
});
export const setDockedItemAtom = atom(null, (get, set, payload) => {
    const { id, position } = payload;
    set(dockedItemsAtom, { ...get(dockedItemsAtom), [id]: position });
});
export const resetDockingAtom = atom(null, (get, set) => {
    set(dockedAtom, true);
    set(positionAtom, defaultPosition);
    set(dockedItemsAtom, {});
});
// Persistence atoms
export const saveDockingToStorageAtom = atom(null, async (get) => {
    try {
...
export const loadDockingFromStorageAtom = atom(null, async (get, set) => {
    try {
        const userId = (await supabase.auth.getUser()).data.user?.id;
        ...
        if (userId) {
            ...
            if (!error && data) {
                ...
                set(dockedItemsAtom, dockedItems);
...
export const initializeDockingAtom = atom(null, async (get, set) => {
