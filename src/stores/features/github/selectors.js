...
// Data Selectors
export const useGitHubBranches = (repoId) => useGitHubStore((state) => state.branches[repoId] || []);
export const useGitHubCommits = (repoId) => useGitHubStore((state) => state.commits[repoId] || []);
export const useGitHubIssues = (repoId) => useGitHubStore((state) => state.issues[repoId] || []);
export const useGitHubPullRequests = (repoId) => useGitHubStore((state) => state.pullRequests[repoId] || []);
// Sync Status Selectors
export const useGitHubSyncStatus = (repoId) => useGitHubStore((state) => state.syncStatus[repoId] || {
    status: 'idle',
    lastSynced: null,
    error: null
});
export const useGitHubIsSyncing = (repoId) => useGitHubStore((state) => state.syncStatus[repoId]?.status === 'syncing');
export const useGitHubSyncError = (repoId) => useGitHubStore((state) => state.syncStatus[repoId]?.error);
// UI State Selectors
...
export const useGitHubRepositoryData = (repoId) => {
