...
import type { GitHubActions, GitHubState } from './types';
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        toggleAutoSync: async (repoId: string, enabled: boolean) => {
          set({ isLoading: true, error: null });
          try {
            const { error } = await supabase
              .from('github_repositories')
            ...
            set(state => ({
              repositories: state.repositories.map(repo =>
                repo.id === repoId ? { ...repo, auto_sync: enabled } : repo
              )
            }));
          } catch (error) {
        ...
        fetchCommits: async (repoId: string, branch?: string) => {
          set({ isLoading: true, error: null });
          try {
            const { data, error } = await supabase.functions.invoke('github-repo-management', {
        ...
        setShowProfileDialog: (show: boolean) => set({ showProfileDialog: show }),
        setShowConnectDialog: (show: boolean) => set({ showConnectDialog: show }),
        setShowDisconnectDialog: (show: boolean) => set({ showDisconnectDialog: show }),
        setShowAccountSwitcher: (show: boolean) => set({ showAccountSwitcher: show }),
