...
export const useSettingsStore = create()(devtools(persist((set) => ({
    ...
    }), false, { type: 'UPDATE_PREFERENCES', payload: updates }),
    saveDashboardLayout: (layout) => set({ dashboardLayout: layout }, false, { type: 'SAVE_LAYOUT', payload: layout }),
    updateNotifications: (settings) => set((state) => ({
        notifications: { ...state.notifications, ...settings },
    }), false, { type: 'UPDATE_NOTIFICATIONS', payload: settings }),
    updateCacheSettings: (settings) => set((state) => ({
        cache: { ...state.cache, ...settings },
    }), false, { type: 'UPDATE_CACHE_SETTINGS', payload: settings }),
