...
export function ProjectOnboardingDialog({ isOpen, onClose, onComplete }) {
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 0:
                return (_jsx(ProjectDetailsStep, { projectDetails: projectDetails, setProjectDetails: setProjectDetails }));
            case 1:
                return (_jsx(RepositoryStep, { projectDetails: projectDetails, repoDetails: repoDetails, setRepoDetails: setRepoDetails }));
            case 2:
                // If importing, show summary. Otherwise show GitHub integration
                return projectDetails.isImporting ? (_jsx(SummaryStep, { projectDetails: projectDetails, repoDetails: repoDetails })) : (_jsx(GitHubIntegrationStep, { projectName: repoDetails.repoName || projectDetails.name.toLowerCase().replace(/\s+/g, '-'), isGithubConnected: isConnected, githubUsername: githubUsername, onPrevious: handlePreviousStep, onContinue: handleNextStep, isSubmitting: isLoading }));
            case 3:
                return (_jsx(SummaryStep, { projectDetails: projectDetails, repoDetails: repoDetails }));
            default:
                return null;
        }
    };
    return (_jsx(Dialog, { open: isOpen, onOpenChange: (open) => !open && handleClose(), children: _jsx(DialogContent, { className: "sm:max-w-[550px]", children: _jsxs("div", { className: "space-y-6", children: [_jsx(OnboardingSteps, { steps: steps, currentStep: currentStep }), _jsx("div", { className: "py-4 min-h-[300px]", children: renderStepContent() }), _jsxs("div", { className: "flex justify-between", children: [_jsx(Button, { variant: "outline", onClick: currentStep === 0 ? handleClose : handlePreviousStep, disabled: isLoading, children: currentStep === 0 ? "Cancel" : "Back" }), currentStep < steps.length - 1 ? (_jsx(Button, { onClick: handleNextStep, disabled: isLoading || (currentStep === 2 && !projectDetails.isImporting && !isConnected), children: "Next" })) : (_jsx(Button, { onClick: handleCreateProject, disabled: isLoading, className: "bg-green-600 hover:bg-green-700", children: isLoading ? "Creating..." : "Create Project" }))] })] }) }) }));
}
