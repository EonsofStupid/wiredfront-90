...
export const clearMiddlewareStorage = () => {
    try {
        ...
        const zustandKeys = allKeys.filter(key => key.includes('zustand') ||
            key.includes('chat-') ||
        ...
        const zustandSessionKeys = sessionKeys.filter(key => key.includes('zustand') ||
            key.includes('chat-') ||
            key.includes('provider-') ||
            key.includes('session-'));
...
export const useChatStore = create()(devtools((set, get) => ({
    ...
    setFeatureState: (featureName, isEnabled) => {
        set(state => ({
            features: {
                ...state.features,
