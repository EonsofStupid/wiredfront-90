...
import { GithubMetrics } from "../types/github-settings";
...
export function useGitHubToken() {
  ...
  const saveGithubToken = async (newGithubToken: string, onGithubTokenChange: (value: string) => void) => {
    if (!newGithubToken) {
      toast.error("Please enter a GitHub token");
      return;
    }
  ...
  const syncGithubMetrics = async () => {
    ...
    try {
      ...
      if (data?.results && data.results.length > 0) {
        const result = data.results.find((r: any) => r.status === 'synced');
        if (result) {
          setMetrics({
