...
export function useProjectOverview() {
    // Get existing hooks
    const { projects, activeProject, activeProjectId, isLoadingProjects } = useProjects();
    // Dialog management
    const { isConnectDialogOpen, setIsConnectDialogOpen, isDisconnectDialogOpen, setIsDisconnectDialogOpen, isImportModalOpen, setIsImportModalOpen } = useGitHubDialogs();
    // GitHub connection
    const { isConnected, isChecking, connectionStatus, githubUsername, linkedAccounts, connectGitHub, disconnectGitHub, checkConnectionStatus } = useGitHubConnection();
    // Project indexing
    const { isIndexing, recentlyImportedProject, simulateIndexing } = useProjectIndexing();
    // Project actions
    const { handleAddProject, handleImportProject, handleDeleteProject, setActiveProject } = useProjectActions();
    // GitHub actions
    const { handleGitHubConnect, handleOpenImportModal } = useGitHubActions(setIsConnectDialogOpen, setIsImportModalOpen);
    // Handle import project with indexing simulation
    const handleProjectImport = (projectId) => {
