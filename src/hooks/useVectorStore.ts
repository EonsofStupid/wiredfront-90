...
import { 
  ...
  createVectorConfiguration
} from '@/stores/features/vector/store';
...
export function useVectorStoreManagement() {
  ...
  useEffect(() => {
    const fetchConfigurations = async () => {
      ...
      try {
        ...
        const configs: VectorConfiguration[] = (data || []).map(item => ({
          id: item.id,
          name: item.name,
   ...
   */
  const createConfig = async (config: NewVectorConfig): Promise<VectorConfiguration | null> => {
    try {
      setLoading(true);
   ...
   */
  const updateConfig = async (id: VectorConfigId, updates: Partial<VectorConfiguration>): Promise<boolean> => {
    try {
      setLoading(true);
   ...
   */
  const deleteConfig = async (id: VectorConfigId): Promise<boolean> => {
    try {
      setLoading(true);
   ...
   */
  const setActiveConfig = async (id: VectorConfigId): Promise<boolean> => {
    try {
      setLoading(true);
