...
// Types for performance metric data
...
export type MetricType = 'apiLatency' | 'cpuUsage' | 'memory' | 'requestCount';
...
export const generateSampleData = (): PerformanceDataPoint[] => {
  const now = new Date();
  const data = [];
...
export function usePerformanceChartData() {
  const [data, setData] = useState<PerformanceDataPoint[]>(() => generateSampleData());
  const [activeMetric, setActiveMetric] = useState<MetricType>('apiLatency');
