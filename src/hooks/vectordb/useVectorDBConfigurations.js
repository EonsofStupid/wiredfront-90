...
import { createVectorConfigId } from '@/types/domain/vector';
import { toast } from 'sonner';
/**
 * Hook for managing vector database configurations
 */
export const useVectorDBConfigurations = () => {
    const { configurations, isLoading, selectedConfig, error, setConfigurations, addConfiguration, updateConfiguration, deleteConfiguration, setSelectedConfig, setLoading, setError } = useVectorStore();
    // Load configurations
    useEffect(() => {
        ...
        try {
            ...
            if (newConfig) {
                ...
                const transformedConfig = {
                    ...
                    storeType: newConfig.store_type,
                    config: configObj,
    ...
    const updateConfig = async (id, updates) => {
        ...
        try {
            ...
            if (updates.storeType)
                dbUpdates.store_type = updates.storeType;
            if (updates.config)
                dbUpdates.config = updates.config;
            if (updates.isActive !== undefined)
                dbUpdates.is_active = updates.isActive;
            if (updates.clusterInfo)
                dbUpdates.cluster_info = updates.clusterInfo;
            if (updates.endpointUrl)
                dbUpdates.endpoint_url = updates.endpointUrl;
            if (updates.grpcEndpoint)
                dbUpdates.grpc_endpoint = updates.grpcEndpoint;
            if (updates.readOnlyKey)
                dbUpdates.read_only_key = updates.readOnlyKey;
            if (updates.environment)
                dbUpdates.environment = updates.environment;
            if (updates.indexName)
                dbUpdates.index_name = updates.indexName;
