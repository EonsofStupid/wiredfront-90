...
export function useProviderChanges() {
    ...
    const { currentProvider, availableProviders, updateCurrentProvider, updateAvailableProviders } = useChatStore();
     ...
     */
    const changeProvider = useCallback(async (newProviderId, reason) => {
        if (!user?.id || isChanging)
            return false;
     ...
     */
    const rollbackToProvider = useCallback(async (historyEntryId) => {
        if (!user?.id || isChanging)
            return false;
