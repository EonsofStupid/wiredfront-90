...
import { Session, DBSession, ChatMode } from '@/types/chat';
...
export function useSessionManager() {
  ...
  const refreshSessions = useCallback(async () => {
    try {
      ...
      const formattedSessions = (data || []).map((session: DBSession): Session => ({
        id: session.id,
        title: session.title,
        ...
        metadata: session.metadata ? 
          (typeof session.metadata === 'string' ? 
            JSON.parse(session.metadata) : session.metadata) as Record<string, any> 
          : {},
        context: session.context ?
          (typeof session.context === 'string' ?
            JSON.parse(session.context) : session.context) as Record<string, any>
          : {}
      }));
  ...
  const clearSessions = useCallback(async (preserveCurrent: boolean = true) => {
    try {
    ...
    try {
      ...
      if (deletedCount > 0) {
        ...
        setSessions(prev => prev.filter(s => {
          const lastAccessed = new Date(s.last_accessed || s.created_at || Date.now());
