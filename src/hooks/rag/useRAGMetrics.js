...
export function useRAGMetrics() {
    ...
    const { data: ragInfo, isLoading: isLoadingRAGInfo, error: ragInfoError, refetch: refetchRAGInfo } = useQuery({
        queryKey: ['rag-info'],
        queryFn: () => RAGMetricsService.getUserRAGMetrics(),
        refetchOnWindowFocus: false,
    });
    // Check if premium RAG is available
    const { data: isPremiumAvailable, isLoading: isCheckingPremium } = useQuery({
    ...
    return {
        ...
        isAtVectorLimit: () => {
            if (!ragInfo)
                return false;
            return ragUtils.isAtVectorLimit(ragInfo.metrics.vectorCount, ragInfo.limits.maxVectors);
        },
        getUsagePercentage: () => {
            if (!ragInfo || !ragInfo.limits.maxVectors)
                return 0;
            return ragUtils.calculateUsagePercentage(ragInfo.metrics.vectorCount, ragInfo.limits.maxVectors);
        }
    };
}
