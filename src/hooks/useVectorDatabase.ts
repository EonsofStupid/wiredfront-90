...
import { addVectorConfiguration, formatNewVectorConfig } from '@/stores/features/vector/utils';
import { NewVectorConfig } from '@/types/store/features/vector/types';
import { VectorConfigId, VectorConfiguration } from '@/types/domain/vector/types';
...
export function useVectorDatabase() {
  ...
  const createConfiguration = useCallback(async (configInput: NewVectorConfig) => {
    setIsLoading(true);
  ...
  const updateConfigurationById = useCallback(async (id: VectorConfigId, updates: Partial<VectorConfiguration>) => {
    setIsLoading(true);
  ...
  const deleteConfigurationById = useCallback(async (id: VectorConfigId) => {
    setIsLoading(true);
