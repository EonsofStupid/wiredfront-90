...
import { LLMProviderInterface } from '@/services/llm/providers/base/ProviderInterface';
import { logger } from '@/services/chat/LoggingService';
import { ChatOptions, ImageOptions, DevOptions } from '@/types/providers/provider-types';

export function useChatProvider() {
  const { currentProvider } = useChatStore();
  const [provider, setProvider] = useState<LLMProviderInterface | null>(null);
  ...
  const generateText = useCallback(async (prompt: string, options?: ChatOptions) => {
    if (!provider) {
  ...
  const generateCode = useCallback(async (prompt: string, options?: DevOptions) => {
    if (!provider) {
      return "No provider selected.";
    }
