 ...
 */
export function useChatMessages({ sessionId, autoSubscribe = true } = {}) {
    ...
    const sendMessageToCurrentSession = useCallback(async (content, role = 'user') => {
        if (!sessionId) {
    ...
    const retryMessage = useCallback(async (messageId) => {
        ...
        if (!message || message.message_status !== 'failed')
            return;
        // Update status to pending
        updateMessage(messageId, { message_status: 'pending' });
        try {
            // Resend the message
            const newId = await sendMessage(message.content, message.session_id || sessionId || '', message.role);
